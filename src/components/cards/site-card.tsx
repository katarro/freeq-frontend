...
export default function SiteCard({
  ...
  return (
    <div className="relative">
      ...
      <Link href={`/user/business/${slug ?? ''}`} className="absolute inset-0 z-10" />
        ...
        <CardHeader className="grid grid-cols-[auto_1fr_auto] items-center gap-x-3 gap-y-0 pt-3">
          <figure className="w-10 h-10 overflow-hidden rounded-full">
            <Image
              ...
              quality={100}
              src={siteImage ? siteImage : 'images/sites/empresa-demo.png'}
              alt={title ?? ''}
              width={40}
              height={40}
            />
          ...
          >
            <svg
              className={cn('!w-6 !h-6 fill-transparent', isLiked && 'fill-primary')}
              width="23"
              height="21"
              viewBox="0 0 23 21"
              fill="inherit"
