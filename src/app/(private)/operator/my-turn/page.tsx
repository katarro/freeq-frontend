...
import { Clock, Calendar, Coffee, Timer, User, MapPin, Play, Pause, Plus } from 'lucide-react';
...
export default function TurnoPage() {
  ...
  const getWorkedHours = () => {
    const worked = (currentTime.getTime() - shiftStartTime.getTime()) / 1000 / 60 / 60 - breakTime / 60;
    return Math.max(0, worked).toFixed(1);
  };

  const getRemainingHours = () => {
    const remaining = (shiftEndTime.getTime() - currentTime.getTime()) / 1000 / 60 / 60;
    return Math.max(0, remaining).toFixed(1);
  };

  const getShiftProgress = () => {
    const total = (shiftEndTime.getTime() - shiftStartTime.getTime()) / 1000 / 60 / 60;
        ...
        <div className="grid gap-4 md:grid-cols-4">
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${
                  isOnBreak ? 'text-yellow-600' : isActive ? 'text-green-600' : 'text-red-600'
                }`}
              >
                {isOnBreak ? 'En Descanso' : isActive ? 'Activo' : 'Inactivo'}
              </div>
              <p className="text-xs text-muted-foreground">
                {isOnBreak ? `${breakTime} min en descanso` : isActive ? 'Disponible para atender' : 'No disponible'}
              </p>
            </CardContent>
          </Card>
          <Card>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">Quedan {getRemainingHours()}h</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Gestiona tu disponibilidad y descansos</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {isOnBreak && (
                <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-yellow-800">En Descanso</span>
              ...
              {!isActive && !isOnBreak && (
                <div className="p-4 bg-red-50 rounded-lg border border-red-200">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-red-800">Atención Pausada</span>
                    <Badge variant="outline" className="border-red-600 text-red-600">
                      Inactivo
                    </Badge>
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">No recibirás nuevos clientes hasta reactivar</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm">{shiftStartTime.toLocaleTimeString()}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Hora de fin:</span>
                <span className="text-sm">{shiftEndTime.toLocaleTimeString()}</span>
              </div>
              <div className="flex justify-between items-center">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                ...
                <span className="text-sm text-muted-foreground">11:00 - 11:15</span>
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-muted rounded-lg">
                ...
                <span className="text-sm text-muted-foreground">13:00 - 13:30</span>
              </div>
              ...
              {isOnBreak && (
                <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                  ...
                  <span className="text-sm text-yellow-600">{breakStartTime?.toLocaleTimeString()} - Activo</span>
                </div>
              )}
            </div>
          </CardContent>
