...
export default function CardsOverview() {
  const allQueues = [
    ...queueConfigs.physical,
    ...queueConfigs.virtual,
    ...queueConfigs.mixed,
  ];

  const totalActiveQueues = allQueues.filter(
    (q) => q.status === 'Activa',
  ).length;
  const totalQueues = allQueues.length;
  const totalClientsInQueue = allQueues.reduce(
    (sum, q) => sum + (q.currentQueue ?? 0),
    0,
  );
  const totalCapacity = allQueues.reduce(
    (sum, q) => sum + (q.capacity ?? 0),
    0,
  );
