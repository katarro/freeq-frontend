...
export default function SubsidiaryTable({ data: initialData }: SubsidiaryTableProps) {
  const [data, setData] = React.useState<SubsidiaryValues[]>(initialData);
  ...
  const [subsidiaryToDeleteId, setSubsidiaryToDeleteId] = React.useState<number | null>(null);
  const [isDeleting, setIsDeleting] = React.useState(false);

  const handleDeleteClick = (subsidiaryId: number) => {
    setSubsidiaryToDeleteId(subsidiaryId);
  ...
  const confirmDelete = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (subsidiaryToDeleteId === null) return;

    setIsDeleting(true);
    try {
      await wait(3000);

      setData((prevData) => prevData.filter((subsidiary) => subsidiary.id !== subsidiaryToDeleteId));
      toast.success(`Sucursal con ID ${subsidiaryToDeleteId} eliminada.`);
    } catch (error) {
      console.error('Error al eliminar la sucursal:', error);
      toast.error('Ocurrió un error al eliminar la sucursal. Inténtalo de nuevo.');
    } finally {
      setIsDeleting(false);
      setIsAlertDialogOpen(false);
      setSubsidiaryToDeleteId(null);
    }
  };
    ...
    <>
      <div className="w-full gap-4 grid">
        <div className="grid lg:grid-cols-[auto_auto] flex-col gap-4 lg:flex-row items-center lg:justify-between">
          <Input
            placeholder="Buscar sucursales por nombre..."
                    ...
                    >
                      {column.id === 'id'
                        ...
                        : column.id === 'name'
                          ...
                          : column.id === 'address'
                            ? 'Dirección'
                            : column.id === 'branchManager'
                              ? 'Jefe de Sucursal'
                              : column.id === 'executives'
                                ? 'Ejecutivos'
                                : column.id === 'status'
                                  ? 'Estado'
                                  : column.id === 'acciones'
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              (ID: {subsidiaryToDeleteId}) de nuestros servidores.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
