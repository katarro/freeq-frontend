...
export default function OperatorTable({ data: initialData }: Props) {
  ...
  const [operatorToDeleteId, setOperatorToDeleteId] = useState<number | null>(null);
  ...
  const confirmDelete = async (event: MouseEvent<HTMLButtonElement>) => {
    ...
    if (operatorToDeleteId === null) return;

    setIsDeleting(true);
    try {
      await wait(3000);
      setData((prevData) => prevData.filter((operator) => operator.id !== operatorToDeleteId));
      toast.success(`Operador con ID ${operatorToDeleteId} eliminado.`);
    } catch (error) {
      console.error('Error al eliminar el operador:', error);
      toast.error('Ocurrió un error al eliminar el operador. Inténtalo de nuevo.');
    } finally {
      setIsDeleting(false);
      ...
      <AlertDialog open={isAlertDialogOpen} onOpenChange={setIsAlertDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción no se puede deshacer. Esto eliminará permanentemente al operador
              (ID: {operatorToDeleteId}) de nuestros servidores.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
