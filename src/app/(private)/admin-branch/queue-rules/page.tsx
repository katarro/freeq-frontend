    ...
    switch (type) {
      case 'tiempos':
        return timeRules.filter(
          (rule) =>
            rule.name?.toLowerCase().includes('tiempo') ||
            false ||
            rule.name?.toLowerCase().includes('alerta') ||
            false ||
            rule.description?.toLowerCase().includes('tiempo') ||
            false,
        );
      case 'ausencias':
        return absenceRules.filter(
          (rule) =>
            rule.name?.toLowerCase().includes('ausencia') ||
            false ||
            rule.name?.toLowerCase().includes('descanso') ||
            false ||
            rule.description?.toLowerCase().includes('ausencia') ||
            false ||
            rule.description?.toLowerCase().includes('descanso') ||
            false,
        );
      case 'prioridad':
        return priorityRules.filter(
          (rule) =>
            rule.name?.toLowerCase().includes('vip') ||
            false ||
            rule.name?.toLowerCase().includes('adulto') ||
            false ||
            rule.name?.toLowerCase().includes('discapacidad') ||
            false ||
            rule.description?.toLowerCase().includes('prioridad') ||
            false,
        );
      default:
        return [];
          ...
          <TabsContent value="prioridad" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Sistema de prioridades para diferentes tipos de clientes
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
