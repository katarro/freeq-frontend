...
export default function CompaniesTable({ data: initialData }: CompaniesTableProps) {
  const [data, setData] = React.useState<CompanyValues[]>(initialData);
  ...
  const [companyToDeleteId, setCompanyToDeleteId] = React.useState<number | null>(null);
  ...
  const confirmDelete = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (companyToDeleteId === null) return;

    setIsDeleting(true);
    try {
      await wait(3000);

      setData((prevData) => prevData.filter((company) => company.id !== companyToDeleteId));
      toast.success(`Empresa con ID ${companyToDeleteId} eliminada.`);
    } catch (error) {
      console.error('Error al eliminar la empresa:', error);
      toast.error('Ocurrió un error al eliminar la empresa. Inténtalo de nuevo.');
    } finally {
      setIsDeleting(false);
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              (ID: {companyToDeleteId}) de nuestros servidores.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
